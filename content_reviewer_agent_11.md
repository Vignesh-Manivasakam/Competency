...
## Plan 11 of 21 — Competency Intelligence Platform MVP

---

### Objective

Implement the Content Reviewer Agent — the quality gate before any generated content reaches the employee. Checks accuracy, clarity, pedagogical soundness, and scope adherence. Uses scoring thresholds to APPROVE, REVISE, or REJECT content, with self-correction capability and fallback to template content.

### Prerequisites

- **Plan 5** (LangGraph Orchestrator) — BaseAgent class
- **Plan 10** (Content Generator Agent) — Content to review

### Spec References

| Section | Content |
|---------|---------|
| §3 Content Reviewer Agent | Full spec: inputs, outputs, thresholds, retry logic |
...
├── agents/
│   └── content_reviewer.py     # Agent implementation
└── prompts/
    └── content_reviewer.txt    # Quality evaluation prompt
```
...
class ContentReviewerAgent(BaseAgent):
    ...
    async def process(self, state: AgentState) -> dict:
        ...
        # Two evaluations for self-consistency (spec: "two separate evaluations")
...
1. **APPROVE**: Content scoring >= 0.82 → `review_decision = "APPROVE"`
2. **REVISE**: Score 0.65-0.82 → self-corrects with revised_content
3. **REJECT**: Score < 0.65 → triggers re-generation
4. **Self-consistency**: Two evaluations averaged
5. **Template fallback**: `get_template_content()` returns valid content for each type
6. **Integration**: Works as LangGraph `review_content` node

### Notes & Gotchas

- **GPT-4o-mini**: Cheaper for critic role; acceptable accuracy for review
- **Two LLM calls**: Self-consistency check; consider making optional for cost savings
- **Retry count**: Tracked in AgentState; route_after_review checks `retry_count < 3`
- **Template content**: Last resort — always works but not personalized
