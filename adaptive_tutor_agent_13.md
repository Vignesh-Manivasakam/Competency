...
## Plan 13 of 21 — Competency Intelligence Platform MVP

---

### Objective

Implement the Adaptive Tutor Agent — drives the live learning session using Socratic dialogue. Presents content, interprets employee responses, decides next interaction (CONTINUE/REINFORCE/ADVANCE/COMPLETE/ESCALATE), adjusts difficulty, and maintains session coherence across turns.

### Prerequisites

- **Plan 5** (LangGraph Orchestrator) — BaseAgent, AgentState
- **Plan 10** (Content Generator) — Content to deliver
- **Plan 9** (Assessment Scoring) — Assessment signals

### Spec References

| Section | Content |
|---------|---------|
| §3 Adaptive Tutor Agent | Full spec: inputs, outputs, Socratic dialogue pattern |
...
├── agents/
│   └── adaptive_tutor.py      # Agent implementation
└── prompts/
    └── adaptive_tutor.txt     # Socratic dialogue prompt
```
...
1. **Socratic response**: Tutor acknowledges before correcting; asks guiding questions
2. **Action decision**: Low score → REINFORCE, high score → ADVANCE, threshold → COMPLETE
3. **Difficulty adjustment**: Correct delta (-1, 0, +1) based on performance
4. **Session coherence**: References previous messages in context
5. **Message history**: Last 10 messages included in prompt for continuity
6. **Persona**: Maintains consistent senior expert persona

### Notes & Gotchas

- **GPT-4o**: Needs dialogue coherence; more expensive but essential for quality
- **Message accumulation**: LangGraph's `add_messages` annotation handles this
- **Context window**: Limit to last 10 messages to prevent overflow
- **Streaming**: In production, stream tutor responses via WebSocket for better UX
